...
# The bus reduction mapping shows which buses were reduced to which parent buses:

get_bus_reduction_map(reduction_data)

# The reverse bus search mapping maps each reduced bus to its ultimate parent:

PNM.get_reverse_bus_search_map(reduction_data)

# The removed arcs (branches):

PNM.get_removed_arcs(reduction_data)

# ## Protecting Specific Buses from Reduction

# In some cases, you may want to preserve certain buses even if they are radial. This can be done by passing the `irreducible_buses` keyword to the matrix constructor:

## Apply radial reduction while protecting buses 8 and 14 (buses must exist in the system)
ybus_protected = Ybus(
    sys;
    network_reductions = NetworkReduction[RadialReduction()],
    irreducible_buses = [8, 14],
);

## Buses 8 and 14 are preserved from reduction
