...
# Call the same element after the row has been stored:
...
# Both reduction types allow you to specify buses that should not be eliminated by passing the `irreducible_buses` keyword to the matrix constructor:

## Protect specific buses from radial reduction
v_ptdf_protected = VirtualPTDF(
    sys_2;
    network_reductions = NetworkReduction[RadialReduction()],
    irreducible_buses = [1, 2],
);
