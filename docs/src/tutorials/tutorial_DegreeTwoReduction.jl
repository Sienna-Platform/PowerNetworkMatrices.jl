...
# The series branch mapping shows how multiple branches were combined into composite branches:
...
# The removed arcs (series branches that were combined):
...
# You can protect certain buses from reduction even if they have degree two, by passing the `irreducible_buses` keyword to the matrix constructor:

## Apply degree-two reduction while protecting bus 115 (if it exists in the system)
ybus_protected = Ybus(
    sys;
    network_reductions = NetworkReduction[DegreeTwoReduction()],
    irreducible_buses = [115],
);
reduction_data_protected = get_network_reduction_data(ybus_protected);

# Compare with the unprotected case:
