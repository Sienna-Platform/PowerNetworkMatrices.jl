...
## Numerics gotchas (hard-won; keep the regression tests green)
  ...
  - **`_is_valid_chain` rejects only circular chains.** A chain running parallel to a pre-existing direct or parallel arc is reduced and absorbed into that arc as a `MixedBranchesParallel` — Ybus is stamped before the map entry is absorbed, so the pre-existing branch's contribution is counted once and the arc-admittance row accumulates every contribution onto the existing row under one label.
