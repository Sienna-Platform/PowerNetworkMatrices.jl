...
## Numerics gotchas (hard-won; keep the regression tests green)
  ...
  - **Sibling degree-two chains share a parallel aggregate.** Chains that resolve to the same *unordered* endpoint pair combine into one `BranchesParallel{BranchesSeries}` routed to `parallel_branch_map`, so traversal direction cannot split a sibling group across two arcs.
  - **`BranchesSeries` carries an `arc_key`.** That gives a chain an arc identity of its own, so it can sit as a member of a `BranchesParallel` or `MixedBranchesParallel` instead of only ever being a standalone `series_branch_map` entry.
  - **`_is_valid_chain` rejects only circular chains.** A chain running parallel to a pre-existing direct or parallel arc is reduced and absorbed into that arc as a `MixedBranchesParallel` — Ybus is stamped before the map entry is absorbed, so the pre-existing branch's contribution is counted once and the arc-admittance row accumulates onto the existing row instead of pushing a duplicate label.
