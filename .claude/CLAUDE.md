...
Degree-two chains are ordinary aggregates, not a separate structure:
  ...
  - `BranchesSeries` carries its own `arc_key`, giving it an arc identity so it can either stand alone in `series_branch_map` or sit as a member of a `BranchesParallel`/`MixedBranchesParallel`.

`IterativeTopologyReduction` composes the primitives instead of reimplementing them:

  - It alternates `RadialReduction` and `DegreeTwoReduction`, each applied through the existing `get_reduction`/`_apply_reduction` path, until one full round eliminates no bus.
    That guarantees the reduced network retains no reducible bus of degree one or two.
  - Do not combine it with a standalone `RadialReduction` or `DegreeTwoReduction`, in either order — both directions throw, because the container can no longer tell a standalone application from the composite's own internal bookkeeping.
  - AC consumers pass `DegreeTwoReduction(; reduce_reactive_power_injectors = false)` into it, the same requirement as the standalone reduction.
  - The radial step has no injector-awareness of its own: a bus that becomes a leaf mid-loop is peeled, and its injection is remapped onto its surviving parent via `reverse_bus_search_map` rather than protected.
    Only `DegreeTwoReduction`'s system-derived set keeps an injector host electrically distinct throughout the loop, so a bus that must stay separate needs `irreducible_buses` too.

`_apply_reduction` writes every composite arc into both `adjacency_data` and `arc_subnetwork_axis`.
That is why `AdjacencyMatrix(ybus)` reflects true post-reduction connectivity rather than the pre-reduction graph, and it is what lets a second `DegreeTwoReduction` pass see the arcs the first pass created.
...
## Source layout
  ...
  - Reductions: `NetworkReduction.jl`, `NetworkReductionData.jl`, `ReductionContainer.jl`, `reduction_helpers.jl`, `radial_reduction.jl`, `degree_two_reduction.jl`, `iterative_topology_reduction.jl` (the spec) + `apply_iterative_topology_reduction.jl` (the loop), `ward_reduction.jl`, `zero_impedance_branch_reduction.jl` (the spec) + `apply_zero_impedance_reduction.jl` (the merge), `BranchesParallel.jl`, `BranchesSeries.jl`, `ThreeWindingTransformerCircuit.jl`, `EquivalentBranch.jl`
...
## Numerics gotchas (hard-won; keep the regression tests green)
  ...
  - **`_is_valid_chain` rejects only circular chains.** A chain running parallel to a pre-existing direct or parallel arc is reduced and absorbed into that arc as a `MixedBranchesParallel` — Ybus is stamped before the map entry is absorbed, so the pre-existing branch's contribution is counted once and the arc-admittance row accumulates every contribution onto the existing row under one label. Absorption moves the pre-existing branch out of `direct_branch_map` into `parallel_branch_map`, and every member of the chain resolves as `:parallel` rather than `:direct` or `:series`. This is the structural fact downstream consumers need: a query keyed on the old direct-map membership or the series classification misses these branches after absorption.
  - **Injector pinning, not any topology guard, is what actually gates degree-two reduction.** `_system_derived_irreducible_buses` removes 77–92% of degree-two buses before chain discovery begins, and "blocked by anything else" is zero across every measured system:
    
    | system      | degree-two buses | candidates after pinning | shape chains realized | shape chains topologically | blocked by pinning | blocked by anything else |
    |:----------- |:---------------- |:------------------------ |:--------------------- |:-------------------------- |:------------------ |:------------------------ |
    | ACTIVSg2000 | 604              | 48                       | 0                     | 3                          | 3                  | 0                        |
    | ACTIVSg10k  | 2132             | 489                      | 0                     | 50                         | 50                 | 0                        |
    | EI 515GW    | 29068            | 2812                     | 1                     | 623                        | 622                | 0                        |
    
    The unpinned "topologically" column recomputes the whole chain partition, so the 623 for EI measures how common the shape is in real topology, not a count of one-for-one extra eliminations.
    `reduce_reactive_power_injectors` still matters at that scale: EI's one realized case exists only under the default `true`, so the AC path, which must pass `false`, gains nothing from this feature there.
