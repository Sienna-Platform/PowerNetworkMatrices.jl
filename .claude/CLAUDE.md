...
- Reductions: `NetworkReduction.jl`, `NetworkReductionData.jl`, `ReductionContainer.jl`, `radial_reduction.jl`, `degree_two_reduction.jl`, `ward_reduction.jl`, `BranchesParallel.jl`, `BranchesSeries.jl`, `ThreeWindingTransformerWinding.jl`, `EquivalentBranch.jl`
...
- **Ybus asymmetry is legitimate** for PhaseShiftingTransformers (`Y[i,j] = −y/t*`, `Y[j,i] = −y/t`) — don't "fix" it. But ZIR column-merge asymmetry was a real DC-PF NaN bug; both directions have regression tests.
...
- **3-winding transformer full outage isolates the star bus** → singular ABA; the pinv islanding path handles it. AppleAccelerate LU silently factorizes singular matrices (garbage results) where KLU throws — prefer KLU whenever singularity is possible.
